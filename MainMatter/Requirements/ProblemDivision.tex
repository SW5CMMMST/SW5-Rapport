...
As the tests in \myref{sec:hardware_tests} have shown, not all packages send are guaranteed to arrive.
...
\myref{fig:network} is an example of a \emph{strongly connected graph} which can be described as: $E \subseteq V \times ]0,1] \times V$ where for any $v_i, v_j$ there exists a directed path from $v_i$ to $v_j$ where all edges in the path has a reliability greater than 0.
