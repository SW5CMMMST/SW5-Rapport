...
A graphical circuit diagram of an Arduino fitted with a transmitter can be seen on \myref{fig:Transmitter}.
...
A graphical circuit diagram of this setup can be seen on \myref{fig:Receiver}.
...
\todo{ what is caption placement even?? -SF}
...
The only place where the results are different \todo{Forstår ikke lige den sætning, skal der står difficult her istedet eller hvad ? -sf } to distinguish from each other is in the graph where the transmitter has an antenna length of 17.3 cm (see \myref{fig:17cm_ant}); here all the different lengths of receiver antennas demonstrates a low package loss percentage compared to the other graphs.
