...
A small subtest has been made to explore if when a package is lost for one device, is the same package lost amongst all other devices.
This is done by simply having one Arduino with a transmitter, with an antenna length of 17.3 cm, transmitting the numbers from zero to 999, and having three Arduinos with an antenna length of 17.3 cm receiving the data. 
...
However external factors can still play a role and cause package loss, this will however be ignored.
...
\begin{description}[labelindent=\parindent]
    ...
    However by testing all the used RF modules in \myref[name]{cha:radio_frequency_module_difference_test}, this source of error is to be considered close to non existent, since all the RF modules used in this test, showed less package loss than 2 \%.
