...
This information is useful at later points during a real-time analysis.\todo{a waht - SF}
The result is therefore a formulae which given the message length outputs the time it takes to send said message. 

\subsubsection*{Theory}
As described in \myref[name]{subsubsec:RadioHead} there is an overhead of 104 bits for each message, along with the 4-to-6 bit conversion.
...
The time it takes to send a message of length x bytes is therefore $f(x)=6.0101 * x + 65.7826$, when the 100 millisecond delay is removed.
