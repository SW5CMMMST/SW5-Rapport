...
First to upload any code to the Arduino, the kind of Arduino being used has to be specified, and how to do this can be seen on \myref{tab:ArduinoScreenshots} (a).
\begin{figure}
\begin{tabular}{cc}
...
Secondly the Port where the Arudino being uploaded to is plugged in needs to be specified, and a screenshot of this can be seen on figure \myref{tab:ArduinoScreenshots} (b).
...
This can be seen on figure \myref{tab:ArduinoScreenshots} (c), the code being run on this screenshot is a simple program which turns on an LED on pin 13 of the Arduino with a varying delay, which is also printed to the serial monitor.
