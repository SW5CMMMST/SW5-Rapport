\section{RadioHead Time Sent Test}\label{subsec:RHTST}
...
The hardware setup used in this test is the same as the one used in the test presented in \myref{subsec:RFMT}.
...
The software used on the Arduinos for this test was different than the other test in \myref{subsec:RFMT}. 
...
These results are gathered over 800 data points, so each point is the average of at least 10 samples. 
In this plot the 100 milliseconds delay as seen in the transmitter code is present.
The average value for 1 byte is 171 milliseconds, and for 60 bytes it its 526 milliseconds. 
