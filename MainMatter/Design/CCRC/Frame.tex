...
The specifics of adding and removing \todo{igen snakker vi om removal, men findes først i ``Improvements to the Protocol'', fjern eller stay? - Troels} devices from the network and how those actions affect the frame will be discussed in sections referring to those individual problems.
...
However if we once again return to the example of a fire alarm, several devices turning on within seconds of each other in a large network seems an unlikely scenario for other than an initial setup of the network, both these problems will be handled in \myref[name]{chap:MDA-CCRC}.
