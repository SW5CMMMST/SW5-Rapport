...
\subsection{Deterministic (non-random) solutions}
First the solutions where the result can always be determined.
These solutions are fast to check for errors as the result of any operation are always predicable.
This means that the number of possible statespace is limited to the amount of start configurations possible.
...
\subsubsection{Kill the network}\label{KtN}
...
\subsection{Random solutions}
The deterministic solutions could be used, but there will always be a chance for ending up in an infinite loop.
To avoid this a random factor can be introduced.
This would mean that eventually any situation that has a chance of going right, will.
Because nothing goes wrong forever when there is a random chance, regardless of how mall, of it going right.
...
\subsubsection{Kill the network (random)}\label{KtNR}
