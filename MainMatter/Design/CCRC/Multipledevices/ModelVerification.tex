...
A comparison has been made for this model with the previous model in \myref{??} \todo{UPPAAL kapitel 7}, when reverting back to allowing the devices to start at the same time.
Query 4 can be run on both models, but on the older simpler model, it will when run with 5 devices yield the result [0.0,0.019], which means that it will probably never succeed.
So a change to the model allows the devices to leave the initial location at any moment between 0 and 30 seconds, and different results occur the devices will now sometimes have enough interval between activation so that a stable network will be created. 
The probability result is [0.75,0.77].
So this shows that if they are seperated in the old model, there is still a good successrate, but as soon as they start at the same time everything collapses.
...
The old model performs poorly, and will not always reach a stable correctly created network, and this is as mentioned only possible whenever there is a interval between activation of the devices.
