...
The UPPAAL model shown in \myref[name]{sec:themodel} has gone through extensive changes to accomodate the challenges posed of multiple devices starting at the same time.
...
A comparison has been made for this model with the previous model in \myref[name]{sec:themodel}, when reverting back to allowing the devices to start at the same time.
...
Therefore this shows that if they are seperated in the old model, there still is a high successrate, but as soon as they start at the same time everything collapses.
