...
\texttt{startup} performs the initial setup of the device, this includes searching for a network or establishing its own network in the case where no network can be detected, a flow diagram of how this process is handled can be seen in \myref{fig:psuedo_flow}.
...
If a network is found prior to \textit{x} exceeding \textit{timeout} the device will process the data and use this to properly attain a slot in the network as is seen on lines 7 - 9 in \myref{lst:setup}.
