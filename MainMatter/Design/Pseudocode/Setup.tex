...
A flow diagram of this setup can be seeon in \myref{fig:psuedo_flow}

\tikzfigure{PsuedoFlowDiagram}{Flow diagram showing how a device acts during the Initialization phase}{psuedo_flow}
 
\bigskip \noindent
\texttt{Inistialize()} which is the function on \myref{lst:setup}performs the initialization of the device.
This includes searching for a network or establishing its own network if no network can be detected.
\myref{lst:setup} introduces are new local variable $t_0$, this constant decides when the device should give up listening for a network and instead establish a network.
...
\begin{lstlisting}[label=lst:setup,style=pseudocode,mathescape=true,caption={Pseudocode example of the special case procedure Initialize()},basicstyle=\ttfamily]
