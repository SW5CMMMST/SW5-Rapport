...
plot 'bars_crazy.dat' every 4 using 1:(($3+$2)/2) with boxes lc rgb '#03A9F4' title "10 %",\
'' every 4    u 1:(($3+$2)/2):(($3-$2)/2) with yerrorbars lt -1 lc rgb '#01579B' t'',\
'' every 4::1 u 1:(($3+$2)/2) with boxes lc rgb '#00BCD4' t "20 %",\
'' every 4::1 u 1:(($3+$2)/2):(($3-$2)/2) with yerrorbars lt -1 lc rgb '#006064' t'',\
'' every 4::2 u 1:(($3+$2)/2) with boxes lc rgb '#009688' t "30 %",\
'' every 4::2 u 1:(($3+$2)/2):(($3-$2)/2) with yerrorbars lt -1 lc rgb '#004D40' t'',\
'' every 4::3 u 1:(($3+$2)/2) with boxes lc rgb '#4CAF50' t "40 %",\
