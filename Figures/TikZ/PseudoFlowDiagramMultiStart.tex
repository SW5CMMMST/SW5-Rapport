...
\begin{tikzpicture}[node distance = 2.8cm, auto]
    ...
    \node [block, below of=found, node distance = 3.5cm] (tocreate) {Create Network};
    ...
    \node [block, right of=myTime, node distance = 3.25cm] (receive) {Receive};
    ...
    \node [decision, left of=payload] (fCountCheck) {Time to die?};
    \node [block, left of=myTime] (fCountAdd) {increment f};
    \node [block, left of=fCountCheck, node distance = 3.25cm] (aAdd) {increment a};
    ...
    \draw [line] (transmit) -| (fCountCheck);
    \draw [line] (fCountCheck) -- node {no} (fCountAdd);
    \draw [line] (fCountAdd) -- (wait);
